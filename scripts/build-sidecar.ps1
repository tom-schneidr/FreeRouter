...
# PyInstaller removes its previous work tree during --clean. Cloud-synced
# workspaces can leave generated bytecode directories read-only, which makes
# that cleanup fail before the actual build starts. These files are disposable
# build output, so clear only their read-only attribute and leave all contents
# intact for PyInstaller to replace.
$CleanupItems = @(
    (Get-Item -LiteralPath $WorkDir -Force)
    (Get-ChildItem -LiteralPath $WorkDir -Force -Recurse -ErrorAction SilentlyContinue)
)
foreach ($CleanupItem in $CleanupItems) {
    $CleanupItem.Attributes = $CleanupItem.Attributes -band (-bnot [IO.FileAttributes]::ReadOnly)
}
